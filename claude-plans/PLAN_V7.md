...
Snapshot of the V7 refactoring plan.
Key changes from V6: Added Part 0 (Tooling & Development Workflow) to document
environment setup, validation commands, and git commit strategy.
...
- One commit per `impl/` module created (steps 1–9)
- One commit for `api.py` + `legacy_api.py` wiring (steps 10–11)
- One commit for `setup.py` / tox / config updates (step 12)
- One commit per compat shim module (steps 13–18)
- One commit for setup.py entry-point updates (step 19)
- One commit per test file migrated (step 20)
