...
Snapshot of the V3 refactoring plan from CLAUDE.md.
Adds: `openedx_keys` lives inside the opaque-keys repo; `make_asset_key` / `make_relative`
method renames; `VersionTree` deprecation.
...
`openedx_keys` is part of the same releasable `opaque-keys` package — no separate
pyproject.toml or version. Add `openedx_keys` everywhere `opaque_keys` appears in
tooling config (tox.ini lint/test targets, pycodestyle, pylint, etc.).
...
`VersionTree` — mark as deprecated (add `DeprecationWarning` to `__init__`), leave in compat `opaque_keys/edx/locator.py`, do not port to `impl/`.
...
8. **`openedx_keys/` setup** — add to `setup.py` `find_packages()`, entry points pointing to new class names; add `openedx_keys` to tox.ini and all linting/test tooling
...
10. **Compat `opaque_keys/edx/locator.py`** — same pattern; deprecate `VersionTree`
