...
def ascii_identifier():
    ...
    return strategies.text(
        alphabet=strategies.sampled_from(
...
@cacheable
def _fields_for_aside_usage_key_v1(
    cls, field
...
@cacheable
def _fields_for_aside_usage_key_v2(
    cls, field
):  # pylint: disable=missing-docstring, function-redefined
    if field == "deprecated":
        return strategies.just(False)
...
@cacheable
def _fields_for_library_locator(
    cls, field
):  # pylint: disable=missing-docstring, function-redefined
...
@cacheable
def _fields_for_library_locator_v2(
    cls, field
):  # pylint: disable=missing-docstring, function-redefined
    if field == "org":
...
@cacheable
def _fields_for_definition_locator(
    cls, field
):  # pylint: disable=missing-docstring, function-redefined
...
@cacheable
def _fields_for_bundle_def_locator(
    cls, field
):  # pylint: disable=missing-docstring, function-redefined
...
def instances_of_key(cls, **kwargs):
    ...
    key_fields = {
        field: kwargs.get(field, fields_for_key(cls, field)) for field in field_names
    }
    return strategies.builds(cls, **key_fields)


@instances_of_key.register(BlockTypeKeyV1)
@cacheable
def _instances_of_block_type_key(
    cls, **kwargs
):  # pylint: disable=missing-docstring, function-redefined

    return strategies.builds(
        cls,
        block_family=kwargs.get(
            "block_family",
            (
                strategies.text(
...
@cacheable
def _instances_of_course_locator(
    cls, **kwargs
):  # pylint: disable=missing-docstring, function-redefined
...
def _instances_of_block_usage(
    ...
    return kwargs.get("course_key", instances_of_key(CourseLocator)).flatmap(
        locator_for_course
    )
...
@cacheable
def _instances_of_library_usage(
    cls, **kwargs
):  # pylint: disable=missing-docstring, function-redefined
...
@cacheable
def _instances_of_deprecated_loc(
    cls, **kwargs
):  # pylint: disable=missing-docstring, function-redefined
    ...
    return strategies.builds(
        ...
        course_key=kwargs.get(
            "course_key",
            instances_of_key(
                CourseLocator, version_guid=strategies.none(), branch=strategies.none(),
            ),
        ),
        block_id=kwargs.get("block_id", allowed_locator_ids()),
        block_type=kwargs.get("block_type", allowed_locator_ids()),
