...
ENCODING_TEXT = strategies.lists(
    strategies.one_of(strategies.text("$:"), strategies.text(),)
).map("".join)
...
class TestAsideKeys(TestCase):
    ...
    @ddt.data(
        *itertools.product(
            [AsideUsageKeyV1, AsideUsageKeyV2,],
            [
                Location.from_string("i4x://org/course/cat/name"),
                BlockUsageLocator(
                    CourseLocator("org", "course", "run"), "block_type", "block_id"
                ),
            ],
            ["aside", "aside_b"],
        )
    ...
    @ddt.data(
        *itertools.product(
            [AsideDefinitionKeyV1, AsideDefinitionKeyV2,],
            [DefinitionLocator("block_type", "abcd1234abcd1234abcd1234"),],
            ["aside", "aside_b"],
        )
    )
    @ddt.unpack
    def test_definition_round_trip_deserialized(
        self, key_class, definition_key, aside_type
    ):
        ...
        *itertools.product(
            ...
            [
                ...
                (
                    "usage_key",
                    BlockUsageLocator(
                        CourseLocator("borg", "horse", "gun"), "lock_type", "lock_id"
                    ),
                ),
                ("block_id", "lock_id"),
