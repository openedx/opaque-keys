...
class TestAssetLocators(TestCase):
    ...
    @ddt.data("/c4x/org/course/asset/path",)
    def test_deprecated_round_trip_asset_location(self, path):
        self.assertEqual(
            path, text_type(AssetKey.from_string(path)),
        )
    ...
    @ddt.data(
        ...
        (
            ...
            "_id.",
            "c4x",
            (CourseLocator("org", "course", "run", "rev", deprecated=True), "ct", None),
        ),
    )
    @ddt.unpack
    ...
    def test_old_charset(self):
        ...
        AssetLocator(
            CourseLocator("a", "b", "c"), "asset", "subs_%20S2x5jhbWl_o.srt.sjson"
        )
        AssetLocator(
            CourseLocator("a", "b", "c"),
            "asset",
    ...
    def test_empty_path(self):
        ...
        with self.assertRaises(InvalidKeyError):
            CourseKey.from_string("course-locator:org+course+run").make_asset_key(
                "asset", ""
            )
    ...
    @ddt.data(
        "asset-v1:UOG+cs_34+Cs128+type@asset", "asset-v1:UOG+cs_34+Cs128",
    )
