...
class TestBlockUsageLocators(LocatorBaseTest):
    ...
    @ddt.data(
        "block-v1:org+course+run+{}@category+{}@name".format(
            BlockUsageLocator.BLOCK_TYPE_PREFIX, BlockUsageLocator.BLOCK_PREFIX
        ),
        "block-v1:org+course+run+{}@revision+{}@category+{}@name".format(
            CourseLocator.BRANCH_PREFIX,
            BlockUsageLocator.BLOCK_TYPE_PREFIX,
            BlockUsageLocator.BLOCK_PREFIX,
        ),
    ...
    def test_valid_locations(
        self, args, kwargs, org, course, run, category, name, revision
    ):  # pylint: disable=unused-argument
        course_key = CourseLocator(
            org=org, course=course, run=run, branch=revision, deprecated=True
        )
        locator = BlockUsageLocator(
            course_key, block_type=category, block_id=name, deprecated=True
        )
    ...
    def test_html_id(self):
        ...
        locator = BlockUsageLocator(
            course_key, block_type="cat", block_id="name:more_name"
        )
        self.assertEqual(locator.html_id(), "name:more_name")

    def test_deprecated_html_id(self):
        course_key = CourseLocator(
            "org", "course", "run", version_guid="rev", deprecated=True
        )
        locator = BlockUsageLocator(
            course_key, block_type="cat", block_id="name:more_name", deprecated=True
        )
    ...
    def test_replacement(self, key):
        ...
        self.assertEqual(
            getattr(
                BlockUsageLocator(course_key, "c", "n", deprecated=True).replace(
                    **kwargs
                ),
                key,
            ),
    ...
    def test_map_into_course_location(self, deprecated_source, deprecated_dest):
        original_course = CourseLocator(
            "org", "course", "run", deprecated=deprecated_source
        )
        new_course = CourseLocator(
            "edX", "toy", "2012_Fall", deprecated=deprecated_dest
        )
        loc = BlockUsageLocator(
            original_course, "cat", "name:more_name", deprecated=deprecated_source
        )
        expected = BlockUsageLocator(
            new_course, "cat", "name:more_name", deprecated=deprecated_dest
        )
    ...
    @ddt.data(
        (
            ...
            "_id.",
            "i4x",
            (CourseLocator("org", "course", "run", "rev", deprecated=True), "ct", "n"),
        ),
        (
            BlockUsageLocator,
            "",
    ...
    def test_deprecated_son_roundtrip(self, key, run):
        self.assertEqual(
            ...
            key.__class__._from_deprecated_son(
                key.to_deprecated_son(), run
    ...
    def test_local_id(self):
        ...
        self.assertEqual(
            BlockUsageLocator(
                CourseLocator("org", "course", "run"), "problem", local_id
            ).block_id,
            local_id,
        )
