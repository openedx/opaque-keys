...
class TestCourseKeys(LocatorBaseTest, TestDeprecated):
    ...
    @ddt.data("foo/bar/baz",)
    def test_deprecated_roundtrip(self, course_id):
        self.assertEqual(course_id, text_type(CourseKey.from_string(course_id)))
    ...
    @ddt.data(
        "org/course/run/foo", "org/course", "org+course+run+foo", "org+course",
    )
    ...
    def test_make_usage_key(self):
        ...
        self.assertEqual(
            text_type(
                BlockUsageLocator(depr_course, "category", "name", deprecated=True)
            ),
    ...
    def test_convert_deprecation(self):
        ...
        self.assertEqual(
            text_type(depr_course.replace(deprecated=False)), text_type(course)
        )
        self.assertEqual(
            text_type(course.replace(deprecated=True)), text_type(depr_course)
        )
    ...
    @ddt.data(
        ...
        "mit.eecs+"
        + CourseLocator.BRANCH_PREFIX
        + "@th%is ",  # pylint: disable=unicode-format-string
        "\ufffd",
    )
    ...
    @ddt.unpack
    def test_course_constructor_trailing_whitespace(
        self, url_with_whitespace_fmt, whitespace
    ):
        with self.assertRaises(InvalidKeyError):
    ...
    def test_course_constructor_url(self):
        ...
        testobj = CourseKey.from_string(
            "course-v1:{}@{}+{}@hw3".format(
                CourseLocator.VERSION_PREFIX, test_id_loc, CourseLocator.BLOCK_PREFIX
            )
        )
        self.check_course_locn_fields(testobj, version_guid=ObjectId(test_id_loc))

    def test_course_constructor_url_package_id_and_version_guid(self):
        test_id_loc = "519665f6223ebd6980884f2b"
        testobj = CourseKey.from_string(
            "course-v1:mit.eecs+honors.6002x+2014_T2+{}@{}".format(
    ...
    def test_course_constructor_package_id_separate_branch(self):
        ...
        expected_urn = "{}+{}+{}+{}@{}".format(
            org, course, run, CourseLocator.BRANCH_PREFIX, test_branch
        )
        testobj = CourseLocator(org=org, course=course, run=run, branch=test_branch)
        self.check_course_locn_fields(
            testobj, org=org, course=course, run=run, branch=test_branch,
        )
    ...
    @ddt.data(
        "i4x://org/course/category/name", "i4x://org/course/category/name@revision"
    )
    def test_make_usage_key_from_deprecated_string_roundtrip(self, url):
        course_key = CourseLocator("org", "course", "run")
        with self.assertDeprecationWarning(count=1):
            self.assertEqual(
                url, text_type(course_key.make_usage_key_from_deprecated_string(url))
            )
