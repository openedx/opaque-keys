...
def valid_key_string(draw):
    ...
    """
    key_type = draw(
        strategies.shared(strategies.sampled_from(KEY_TYPES), key="key_type")
    )
...
def perturbed_by_subsection(draw, string_strategy):
    ...
    for _ in range(iterations):
        range_start = draw(
            strategies.integers(min_value=0, max_value=len(output_string))
        )
        range_end = draw(
            strategies.integers(min_value=range_start, max_value=len(output_string))
        )
        ...
        output_string = (
            output_string[:range_start] + substitution + output_string[range_end:]
        )
...
def perturbed_strings(string_strategy):
    ...
    """
    return perturbed_by_character(string_strategy) | perturbed_by_subsection(
        string_strategy
    )
...
@given(
    ...
    perturbed=perturbed_strings(
        strategies.shared(valid_key_string(), key="diff_serial_diff_key")
    ),
)
...
@example(
    key_type=AssetKey, serialized="/c4x/-/-/-/-", perturbed="c4x/-/-/-/-",
)
@example(
    key_type=UsageKey, serialized="i4x://-/-/-/-@-", perturbed="i4x:/-/-/-/-@-",
)
@example(
    key_type=AssetKey,
    serialized="/c4x/-/-/-/-@0",
